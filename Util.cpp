...
bool CUtil::IsVisible(void* pLocal, void* pEntity, QAngle vStart, QAngle vEnd)
{
	trace_t Trace;
	...
	Iface.EngineTrace->TraceRay(Ray, MASK_SHOT, &Filter, &Trace);
...
bool CUtil::IsHeadshotWeapon(C_Entity* pLocal, CBaseCombatWeapon* pWep)
{
...
DWORD CUtil::FindSignatures(const char* szModuleName, char* szPattern)
{
	HMODULE hModule = GetModuleHandleA(szModuleName);
	PIMAGE_DOS_HEADER pDOSHeader = (PIMAGE_DOS_HEADER)hModule;
	PIMAGE_NT_HEADERS pNTHeaders = (PIMAGE_NT_HEADERS)(((DWORD)hModule) + pDOSHeader->e_lfanew);

	return gSignatures.dwFindPattern(((DWORD)hModule) + pNTHeaders->OptionalHeader.BaseOfCode, ((DWORD)hModule) + pNTHeaders->OptionalHeader.SizeOfCode, szPattern);
}
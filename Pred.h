...
public:
	...
	virtual void	ProcessMovement(C_Entity *pPlayer, CMoveData *pMove);

	virtual void	StartTrackPredictionErrors(C_Entity *pPlayer);
	virtual void	FinishTrackPredictionErrors(C_Entity *pPlayer);
...
public:
	...
	virtual void	RunCommand(C_Entity *player, C_UserCmd *ucmd, IMoveHelper *moveHelper);

	// Internal
public:
	virtual void	SetupMove(C_Entity *player, C_UserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move);
	virtual void	FinishMove(C_Entity *player, C_UserCmd *ucmd, CMoveData *move);
	virtual void	SetIdealPitch(C_Entity *player, const Vector& origin, const Vector& angles, const Vector& viewheight);

	void			CheckError(int commands_acknowledged);

	// Called before and after any movement processing
	void			StartCommand(C_Entity *player, C_UserCmd *cmd);
	void			FinishCommand(C_Entity *player);

	// Helpers to call pre and post think for player, and to call think if a think function is set
	void			RunPreThink(C_Entity *player);
	void			RunThink(C_Entity *ent, double frametime);
	void			RunPostThink(C_Entity *player);
...
private:
	...
	bool			PerformPrediction(bool received_new_world_update, C_Entity *localPlayer, int incoming_acknowledged, int outgoing_command);
	...
	void			RunSimulation(int current_command, float curtime, C_UserCmd *cmd, C_Entity *localPlayer);
	void			Untouch(void);
	void			StorePredictionResults(int predicted_frame);
	bool			ShouldDumpEntity(C_Entity *ent);

	void			SmoothViewOnMovingPlatform(C_Entity *pPlayer, Vector& offset);
	// Data
protected:

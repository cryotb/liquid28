...
public:
	...
	Color GetPlayerColor(C_Entity* pPlayer);
private:
	unsigned long m_Font;
};

class CDrawManager_t : CDrawManager
{
public:
	void Initialize();
	void DrawString(int font, int x, int y, Color clrColor, const wchar_t *pszText);
	void DrawString(int font, int x, int y, Color clrColor, const char *pszText, ...);
	byte GetESPHeight();
	void DrawLine(int x0, int y0, int x1, int y1, Color clrColor);
	void DrawLineEx(int x, int y, int w, int h, Color clrColor);
	void DrawBox(Vector vOrigin, int r, int g, int b, int alpha, int box_width, int radius);
...
//===================================================================================
extern CDrawManager pDrawing;
extern CDrawManager_t pDrawing_t;
//===================================================================================